...
# Initialize the basic FastAPI app
app = FastAPI(
...
# Import database and other dependencies with error handling
try:
    ...
    class SystemStatus(BaseModel):
        ...
        lead_count: Dict[str, Any]  # Changed from Dict[str, int] to allow string error messages
        uptime: int  # Seconds
        started_at: float

    @app.get("/debug")
    async def debug_info():
        """Get debug information about imported modules and configurations"""
        try:
            return {
                "imports": {
                    "database_models": "✓ Imported",
                    "config": "✓ Imported",
                    "utils": "✓ Imported",
                    "logging_setup": "✓ Imported"
                },
                "config": {
            ...
            return {"error": f"Error retrieving debug info: {str(e)}"}
    ...
    async def get_status(db: Session = Depends(get_db)):
        ...
        try:
            ...
            if db_connected:
                ...
                lead_count = {"error": "Database not connected"}
    ...
    async def test_db_connection():
        """Test the database connection and return detailed error information if it fails"""
        try:
            # Try to get a database session
            session = get_session()
            try:
                # Try a simple query to check connection using SQLAlchemy text()
                from sqlalchemy import text
                ...
                return {
                    ...
                    "database_url": config.DATABASE_URL.replace(":123@", ":***@")  # Hide password
                }
            except Exception as e:
                return {
                    "status": "error",
    ...
    # Import and define other routes here...
    
except ImportError as e:
    print(f"Warning: Some modules could not be imported: {e}")
    print("API will run in limited mode with basic health check only")

def start_api():
    """Start the API server"""
    # When running in a thread, we can't use the reload option
    # (signal only works in the main thread)
    uvicorn.run(
        "shared.api:app", 
        host="0.0.0.0",  # Hardcoded for testing
        port=8000,       # Hardcoded for testing
        reload=False     # Disable reload when running in a thread
    )

if __name__ == "__main__":
    # Here we can use reload as we're in the main thread
    uvicorn.run(
        "shared.api:app", 
        host="0.0.0.0",  # Hardcoded for testing
        port=8000,       # Hardcoded for testing
